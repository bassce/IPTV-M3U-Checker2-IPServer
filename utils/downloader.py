...
class Downloader:
    ...
    def downloadTester(self, retry=3, timeout=5):
        chunck_size = 10240
        for i in range(retry):
            try:
                resp = urlopen(self.url, timeout=timeout)
                # max 5s
                ...
                print(f"downloadTester got an error {e}, retry {i+1}, {self.m3url}\n")
                self.receive = -1
                self.startTime = time.time()
        ...
        if len(video_url) > 0:
            try:
                ...
                if video.isOpened():
                    ...
                    if 0x20 <= ord(c1) <= 0x7a and 0x20 <= ord(c2) <= 0x7a and 0x20 <= ord(c3) <= 0x7a and 0x20 <= ord(c4) <= 0x7a:
